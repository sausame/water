...
public class SwipeHelper {
	...
	private int mOffsetY, mStatusBarHeight;
	...
	private View mCurrAnimView = null;
	private boolean mCanCurrViewBeDimissed;
	private float mDensityScale;
	private Context mContext;

	public SwipeHelper(int swipeDirection, Callback callback,
			float densityScale, float pagingTouchSlop, Context context) {
		mCallback = callback;
	...
	private float getAlphaForOffset(View view, int delta) {
		float viewSize = getSize(view);
		final float fadeSize = ALPHA_FADE_END * viewSize;
		float result = 1.0f;
		if (delta >= viewSize * ALPHA_FADE_START) {
			result = 1.0f - (delta - viewSize * ALPHA_FADE_START) / fadeSize;
		} else if (delta < viewSize * (1.0f - ALPHA_FADE_START)) {
			result = 1.0f + (viewSize * ALPHA_FADE_START + delta) / fadeSize;
		}
		return Math.max(0.05f, result);
	}
		...
		if (Build.VERSION.SDK_INT < Build.VERSION_CODES.HONEYCOMB) {
			...
			case MotionEvent.ACTION_DOWN:
				...
				if (mCurrView != null) {
					...
					mOffsetY = (int) ev.getRawY() - (int) ev.getY()
							+ mCurrView.getTop() - mStatusBarHeight;
				}
				break;
			case MotionEvent.ACTION_MOVE:
				if (mCurrView != null) {
					...
					if (Math.abs(delta) > mPagingTouchSlop) {
						...
						startDrag((int) getPos(ev));
						mCurrView.setVisibility(View.INVISIBLE);
						mDragging = true;
					}
				}
				break;
	...
	private void startDrag(int x) {
		...
		mWindowParams.x = x - (int) mInitialTouchPos;
		mWindowParams.y = mOffsetY;
		mWindowParams.alpha = getAlphaForOffset(mCurrAnimView, x
				- (int) mInitialTouchPos);
		mWindowParams.flags = WindowManager.LayoutParams.FLAG_LAYOUT_NO_LIMITS | 0x00000010;

		mWindowManager = (WindowManager) mContext
				.getSystemService(Context.WINDOW_SERVICE);
		mWindowManager.addView(mCurrAnimView, mWindowParams);
	}
	...
	private void drag(int x) {
		...
		mWindowParams.alpha = getAlphaForOffset(mCurrAnimView, x
				- (int) mInitialTouchPos);
		mWindowManager.updateViewLayout(mCurrAnimView, mWindowParams);
	}

	private void stopDrag() {
		if (mCurrAnimView != null && mWindowManager != null) {
			mWindowManager.removeView(mCurrAnimView);
			mCurrAnimView = null;
		}
	}
		...
		if (Build.VERSION.SDK_INT < Build.VERSION_CODES.HONEYCOMB) {
			...
			case MotionEvent.ACTION_CANCEL:
				if (mCurrView != null) {
					...
					mVelocityTracker.computeCurrentVelocity(1000 /* px/sec */,
							maxVelocity);
					float escapeVelocity = SWIPE_ESCAPE_VELOCITY
							* mDensityScale;
					...
					boolean childSwipedFarEnough = DISMISS_IF_SWIPED_FAR_ENOUGH
							&& Math.abs(getPos(ev) - mInitialTouchPos) > 0.4 * getSize(mCurrAnimView);
					boolean childSwipedFastEnough = (Math.abs(velocity) > escapeVelocity)
							&& (Math.abs(velocity) > Math
									.abs(perpendicularVelocity))
							&& (velocity > 0) == (getPos(ev) - mInitialTouchPos > 0);
					...
					boolean dismissChild = mCallback
							.canChildBeDismissed(mCurrView)
							&& (childSwipedFastEnough || childSwipedFarEnough);
					...
					if (dismissChild) {
						...
						// snappity
						mCurrView.setVisibility(View.VISIBLE);
			...
			case MotionEvent.ACTION_MOVE:
				if (mCurrView != null) {
					...
					if (FADE_OUT_DURING_SWIPE && mCanCurrViewBeDimissed) {
						mCurrAnimView
								.setAlpha(getAlphaForOffset(mCurrAnimView));
					}
			...
			case MotionEvent.ACTION_CANCEL:
				if (mCurrView != null) {
					...
					if (dismissChild) {
						// flingadingy
						dismissChild(mCurrView,
								childSwipedFastEnough ? velocity : 0f);
					} else {
						// snappity
						mCallback.onDragCancelled(mCurrView);
